/**
 * @file oled.c
 * @brief 波特律动OLED驱动(CH1116) - 修改为SPI接口
 ...
 * @note
 * 使用流程:
 * 1. STM32初始化SPI完成后调用OLED_Init()初始化OLED. 注意STM32启动比OLED上电快, 可等待20ms再初始化OLED
...
#define OLED_COLUMN_OFFSET 2   // OLED列偏移 由于OLED的显示区域比实际的列数少2列 因此需要偏移2列
...
void OLED_Init() {
  ...
  OLED_SendCmd(0x00); /*设置列起始地址低位 set lower column address*/
  OLED_SendCmd(0x10); /*设置列起始地址高位 set higher column address*/
  ...
  OLED_SendCmd(0x8D); /*设置启动电荷泵 set charge pump enable*/
  OLED_SendCmd(0x14); /*启动DC-DC */
...
/**
 * @brief 将当前显存显示到屏幕上(SPI版本)
 * @note 此函数是移植本驱动时的重要函数 将本驱动库移植到其他驱动芯片时应根据实际情况修改此函数
 */
void OLED_ShowFrame() {
  uint8_t cmd;
  // 全程保持CS低, 一次性传输整帧8页数据, 仅通过DC切换命令/数据模式
  // 避免逐页切换CS导致的CS高脉冲过短(CH1116要求≥100ns), 消除页间数据错乱
  HAL_GPIO_WritePin(SCREEN_CS_GPIO_Port, SCREEN_CS_Pin, GPIO_PIN_RESET);
